  ...
  try {
    ...
    if (data.thong_tin) {
      ...
      if (data.thong_tin.password) {
        ...
        // Hash password mới
        data.thong_tin.password = await bcrypt.hash(
          data.thong_tin.password,
          10
        );
      } else {
    ...
    const updateFields = {};
    if (data.ngay_thu_hoi)
      updateFields.ngay_thu_hoi = new Date(data.ngay_thu_hoi);
    ...
    await ChiTietHanhDong.create({
      loai_hanh_dong: `Cập nhật thông tin đăng nhập tài sản của ${
        data.NguoiNhan || ""
      }`,
      HanhDongId: user.hanh_dong,
    });
