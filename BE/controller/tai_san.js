...
const getTaiSan = async (req, res) => {
  try {
    const results = await TaiSan.getTaiSan(req.query, req.user);
...
const getTaiSanDetailsService = async (req, res) => {
  try {
    const idArray = req.query["nhaCungCapIds[]"];
    const taiSanDetails = await TaiSan.getTaiSanDetailsService(idArray);
    if (!taiSanDetails) {
      return res.status(404).json({ error: "Tài sản không tồn tại" });
    }
    res.status(200).json(taiSanDetails);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
