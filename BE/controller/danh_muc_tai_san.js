...
const getAllDanhMucTaiSanController = async (req, res) => {
  try {
    const data = await DanhMucTaiSan.getAllDanhMucTaiSan(req.user, req.query);
    res.status(201).json({
      status: true,
...
const updateDanhMucTaiSan = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await DanhMucTaiSan.updateDanhMucTaiSan(
      id,
      req.body,
      req.user
    );
